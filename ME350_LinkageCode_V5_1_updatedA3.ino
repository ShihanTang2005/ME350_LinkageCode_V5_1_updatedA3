...
// Shihan Update 4/6: I changed the noise limit from 8 to 11 for near the photosensors and from 15 to 21 
// for near the prox sensors to make the direction detection more robust to noise.
const int lowerNoiseLimit = 11;        // noise limit for near the photosensors
const int upperNoiseLimit = 21;        // noise limit for near the prox sensors
...
void loop() {
  ...
  if ((abs(motorPosition - previousMotorPosition) > MIN_VEL_COMP_COUNT) || (micros() - previousVelCompTime) > MIN_VEL_COMP_TIME){
    ...
    motorVelocity = (float)(motorPosition - previousMotorPosition) * 1000000.0 / (float)(micros() - previousVelCompTime);
    // (speed = delta angle [encoder counts] divided by delta time [seconds])
    //What is the reason for having a minimum proximity sensor value and a maximum proximity sensor value? For example, why is there a PROXIMITYSENSE1_MIN and PROXIMITYSENSE1_MAX?
...
// Shihan Update 4/6: I changed the forward count threshold from 3 to 5 to make
// the direction detection more robust to noise.  
//This means that the controller will take a little bit 
//longer to detect a change in direction, but it should be less
// likely to falsely detect a change in direction due to noise.
  ...
  // printStateToSerial();
  // 打印表头（方便在 Serial Plotter 中看曲线）
  // Serial.print("S1_Raw:"); Serial.print(analogRead(PIN_PROXSENSE1));
  // Serial.print(",");
  
  // Serial.print("Target1_Filtered:"); Serial.print(ProxSensors[TARGET1].currVal);
  // Serial.print("\t");
  // Serial.print("Target2_Filtered:"); Serial.print(ProxSensors[TARGET2].currVal);
  // Serial.print("\t");
  // Serial.print("Target3_Filtered:"); Serial.print(ProxSensors[TARGET3].currVal);
  // Serial.print("\t");
  // Serial.print("Target4_Filtered:"); Serial.print(ProxSensors[TARGET4].currVal);
  
  // Serial.print("\t| ActiveIdx: "); Serial.print(activeTargetIndex + 1);
  // Serial.print("\t| Dir1: "); 
  // if(ProxSensors[0].direction == FORWARD) Serial.print("FWD");
  // else if(ProxSensors[0].direction == STOPPED) Serial.print("STP");
